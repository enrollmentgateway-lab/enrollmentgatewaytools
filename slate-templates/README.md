...
The Record Lookup wrapper does not require a portal query. It fetches the configured Slate JSON service after a first-name, last-name, or SIS-ID search. That service must export `status`, `name`, `email`, `phone`, `pipeline`, `teachingsite`, `birthdate`, `app_created_date`, `decision_code`, `vear`, `term`, `program`, `url`, `app_degree`, `app_round`, `app_status`, `app_term`, `app_year`, `app_f1`, `app_url`, `missing_mats_list`, and `full_mats_list`. The material-list fields are comma-separated plaintext; the interface compares them to mark each full-list item as **Missing** or **Received** when an application is awaiting materials. The `url` export must point to the person record in Slate. The application URL is retained only as an application-presence signal; the interface does not expose an applicant-facing application link.
