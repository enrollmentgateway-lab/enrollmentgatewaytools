...
The Regional Campus portal loads the `campus` values from Queryomatic's Slate prompt/options service, with the current campus-prompt values as a static wrapper fallback, and excludes `Doctor of Ministry`. It calls two JSON services. The general service uses `campus`, `term`, and `year` and supplies Applicant and Student rows with `first`, `last`, `email`, `phone`, `status`, `app_status`, and `program`. The inquiry-only service uses `campus` and supplies `first`, `last`, `email`, and `phone`; the wrapper assigns the `Inquiry` status. Inquiry counts are all-time rather than period-specific. No Liquid query export is required. The interface labels `app_status` as **Application Status**.
