...
The Regional Campus portal loads the `campus` values from Queryomatic's Slate prompt/options service, with the current campus-prompt values as a static wrapper fallback, excludes `Doctor of Ministry`, and calls its JSON records service once per remaining campus with `campus`, `term`, and `year`. No Liquid query export is required. The service must return `first`, `last`, `email`, `phone`, `status`, `app_status`, and `program`; the wrapper supplies the campus and selected period used for each request. The interface labels `app_status` as **Application Status** and filters status drilldowns client-side.
