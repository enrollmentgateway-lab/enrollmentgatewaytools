...
The Regional Campus portal loads the `campus` values from Queryomatic's Slate prompt/options service, with the current campus-prompt values as a static wrapper fallback, and excludes `Doctor of Ministry`. Its general JSON requests use `campus`, `term`, and `year`; its inquiry requests omit term/year and send `campus`, `status=inquiry`, `person_created_date_start`, and `person_created_date_end`. For example, `2026-2027` maps to `2026-01-01` through `2027-12-31`; the all-time view omits both created-date parameters. The service's status filter must be optional: no status parameter returns every status, while `status=inquiry` returns only inquiries. If the dedicated inquiry response is unavailable or empty, inquiry rows from the general response are retained. No Liquid query export is required. The service must return `first`, `last`, `email`, `phone`, `status`, `app_status`, and `program`; the wrapper supplies the campus and selected period used for each request. The interface labels `app_status` as **Application Status**.
