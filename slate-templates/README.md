...
The Student Lookup wrapper expects the `student_lookup` query to export `app_degree`, `app_round`, `app_status`, `app_term`, `app_year`, `app_f1`, and `app_url` for its application-document section. `app_url` must point to the application in Slate.
