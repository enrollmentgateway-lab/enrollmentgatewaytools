...
The Regional Campus wrapper does not require a Liquid query export. It reads `campus` key/value rows from Queryomatic's Slate prompt/options service (falling back to the current static campus-prompt values in the wrapper), excludes `Doctor of Ministry`, then queries the configured regional-campus JSON service for every remaining campus. Non-inquiry calls use `campus`, `term`, and `year`. Inquiry calls omit `term` and `year` and instead use `campus`, `status=inquiry`, `person_created_date_start`, and `person_created_date_end`; an academic year such as `2026-2027` becomes the inclusive created-date range `2026-01-01` through `2027-12-31`. The all-time view omits the created-date parameters. Configure the service's status filter as optional so a missing status returns every status and `status=inquiry` returns only inquiries. If the dedicated inquiry request fails or returns no rows, inquiry rows from the general response are retained as a fallback. The wrapper tags each returned row with the requested campus, combines the responses, and sends the resulting records to the hosted interface. The JSON service must export `first`, `last`, `email`, `phone`, `status`, `app_status`, and `program`; the interface displays `app_status` as **Application Status**. Choosing a campus limits the next page load to that campus's service calls.
